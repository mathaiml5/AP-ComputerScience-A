/*********************************************************************************************************************
 * The Autocomplete GUI is a Swing GUI reference implementation provided by
 * Princeton CS department as part of CS226 course
 * (file is located at: ftp://ftp.cs.princeton.edu/pub/cs226/autocomplete/AutocompleteGUI.java)
 * The file is included as is from the original version in the ftp site except for one small modification to remove the
 * dependency on a custom input file reader library referenced in the line "import edu.princeton.cs.algs4.In;"
 * This was replaced with a simple buffered reader which is more generic and comes with any standard java install and
 * avoid any external dependencies that are not part of standard java distribution
 * Comments by: Vishak Srikanth
 * Original File content inserted below except where noted with comments with my initial "@VS:"
 * ***************************************************************************
...
//@VS Removed the reference to custom input reader library and replaced with a plain input reader
//@VS using buffered reader
//@VS import edu.princeton.cs.algs4.In;
...
public class AutocompleteGUI extends JFrame {
    ...
    private class AutocompletePanel extends JPanel {
        ...
        private JTextField searchText = new JTextField();           // the search bar
        ...
        private JScrollPane scrollPane;                             // the scroll bar on the side of the suggestions
        ...
        public AutocompletePanel(String filename) {
            super();
            Term[] myterms = null;
            //@VS Replaced custom library based file reader with generic file reading based on standard java library
            //@VS buffered input file readers
            try {
                BufferedReader inputFile = new BufferedReader(new FileReader(filename));

                StringTokenizer st = new StringTokenizer(inputFile.readLine());
                //Database File has total # of terms in file as 1st line
                int numTerms = Integer.parseInt(st.nextToken());
                //initialize empty term array
                myterms = new Term[numTerms];
                ...
                for (int i = 0; i < numTerms; i++) {
                    ...
                    myterms[i] = new Term(query, weight);
                }

                /* @VS **** Section with dependency on custom file reader input class from Princeton CS226 algorithms course
                ...
                @VS End of Section with dependency on custom file reader input class from Princeton CS226 algorithms course
                @VS ******/
